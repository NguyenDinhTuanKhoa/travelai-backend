...
function determineCategory(title, category) {
  ...
  if (titleLower.includes('khách sạn') || titleLower.includes('hotel') || 
    ...
    return 'restaurant';
             ...
             titleLower.includes('museum') || titleLower.includes('nhà rông')) {
    return 'historical';
    ...
    return 'beach';
             ...
             titleLower.includes('hồ') || titleLower.includes('măng đen')) {
    return 'countryside';
...
async function fetchKonTum() {
  try {
    await mongoose.connect(process.env.MONGODB_URI);
    console.log('Connected to MongoDB');
    console.log('Starting Kon Tum data collection with Serper.dev...\n');
    console.log('═══════════════════════════════════════════════════════════');
      ...
      try {
        ...
        for (const place of placesData.places) {
          ...
          if (!isWithinProvince(place.latitude, place.longitude, PROVINCE_NAME)) {
            ...
            console.log(`   ⏭️  Skipped "${place.title}" - Outside Kon Tum (${place.latitude.toFixed(4)}, ${place.longitude.toFixed(4)}) - Closer to ${closest.province}`);
            skippedCount++;
            continue;
          }
          ...
          const images = await serperManager.searchImages(`${place.title} Kon Tum`, 3);
          ...
          const destination = {
            name: place.title,
            description: `${place.title} tại Kon Tum - ${place.address || 'Tây Nguyên'}`,
            category: category,
            location: {
              city: PROVINCE_NAME,
    ...
    const totalKonTum = await Destination.countDocuments({ 'location.city': PROVINCE_NAME });
    
    console.log('\n═══════════════════════════════════════════════════════════');
    console.log('🎉 KON TUM DATA COLLECTION COMPLETED!');
    console.log(`💾 Saved: ${savedCount} destinations`);
    console.log(`⏭️  Skipped: ${skippedCount} destinations`);
    console.log(`❌ Errors: ${errorCount}`);
    console.log(`📍 Total Kon Tum destinations in database: ${totalKonTum}`);
    console.log('═══════════════════════════════════════════════════════════\n');
